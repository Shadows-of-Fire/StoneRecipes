...
public class NBTShapelessRecipe extends ShapelessRecipes {
	...
	public static void createNBTRecipe(CraftShapelessRecipe original) {
		List<RecipeChoice> ingred = original.getChoiceList();
	...
	public static <T> int[] findMatches(List<T> inputs, List<? extends Predicate<T>> tests) {
		...
		for (int x = 0; x < elements; x++) {
			...
			if (matched == 1) {
				if (!claim(ret, data, x, elements)) return null; //We failed to claim this index, which means it caused something else to go to 0 matches, which makes the whole thing fail
			}
		}
	...
	private static boolean claim(int[] ret, BitSet data, int claimed, int elements) {
		...
		while (pending.peek() != null) {
			...
			if (used >= elements || used < 0) throw new IllegalStateException("What? We matched something, but it wasn't set in the range of this test! Test: " + test + " Used: " + used);
